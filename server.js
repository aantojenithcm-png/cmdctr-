...
try {
  ...
  // hospitals.json is stored as an object with the actual records
  // inside the "hospitals" array.
  if (Array.isArray(hospitalData)) {
    HOSPITALS = hospitalData;
  } else if (hospitalData && Array.isArray(hospitalData.hospitals)) {
    HOSPITALS = hospitalData.hospitals;
  } else {
    console.error("data/hospitals.json does not contain a hospitals array.");
  }
...
if (method === "GET" && pathname === "/api/incidents/pending") {
  ...
  if (method === "POST" && pathname === "/api/hospitals/recommend") {
    ...
    return sendJson(res, 200, {
      ...
      totalHospitalsLoaded: HOSPITALS.length,
      hospitals
    });
  }
