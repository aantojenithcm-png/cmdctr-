  ...
  document.querySelectorAll("[data-implement]").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      const r = await apiPost(
        ...
        { reportId: btn.dataset.implement, decision: "approve", hospitalId: selectedHospitalByReport[btn.dataset.implement] || null },
        { auth: true, role: "command" }
      );
...
async function renderCitizen() {
  ...
  document.getElementById("main").innerHTML = `
    <div class="citizen-wrap">
      ...
      <div class="citizen-form" style="display:grid; gap:10px;">
        ...
        <div style="position:relative; width:100%;">
          <input id="citizen-location"
            readonly
            placeholder="Tap the location button to get your live location"
            aria-label="Live GPS location"
            style="width:100%; box-sizing:border-box; padding:10px 52px 10px 10px; border-radius:6px; border:1px solid var(--border);
            background:var(--panel2); color:var(--text); cursor:pointer;">

          <button id="citizen-location-btn" type="button"
            aria-label="Use my current location"
            title="Use my current location"
            style="position:absolute; right:6px; top:50%; transform:translateY(-50%); width:38px; height:38px;
            border-radius:50%; border:1px solid var(--border); background:var(--panel); color:var(--teal);
            display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:21px; line-height:1;">
            ⦿
          </button>
        </div>
        ...
        <div id="citizen-location-status"
          ...
          Tap the location button to share your live GPS location.
        </div>
      ...
      <button class="sos-btn" id="sos-btn" disabled style="opacity:.55; cursor:not-allowed;">
  ...
  const locationBtn = document.getElementById("citizen-location-btn");
  ...
  function setReportEnabled(enabled) {
    ...
    sosBtn.style.opacity = enabled ? "1" : ".55";
    sosBtn.style.cursor = enabled ? "pointer" : "not-allowed";
  }
  ...
  function getLiveLocation() {
    ...
    if (!navigator.geolocation) {
      showLocationError("This browser does not support GPS location. Please use a supported browser.");
      return;
    }
    ...
    locationBtn.style.opacity = ".55";
    locationStatus.textContent = "Finding your live location…";
    locationStatus.style.color = "var(--teal)";
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        gpsLatitude = Number(position.coords.latitude);
        gpsLongitude = Number(position.coords.longitude);
        gpsConfirmed = Number.isFinite(gpsLatitude) && Number.isFinite(gpsLongitude);

        if (!gpsConfirmed) {
          showLocationError("GPS returned an invalid location. Please try again.");
          return;
        }

        const accuracy = Number(position.coords.accuracy);
        locationInput.value = `${gpsLatitude.toFixed(6)}, ${gpsLongitude.toFixed(6)}`;
        locationInput.dataset.latitude = String(gpsLatitude);
        locationInput.dataset.longitude = String(gpsLongitude);

        locationStatus.textContent = Number.isFinite(accuracy)
          ? `✓ Live location confirmed • GPS accuracy about ${Math.round(accuracy)} m`
          : "✓ Live GPS location confirmed";
        locationStatus.style.color = "var(--teal)";
      ...
      error => {
        ...
        if (error.code === 1) {
          showLocationError("Location permission was denied. Turn on Location permission for this website and tap the location button again.");
        } else if (error.code === 2) {
          showLocationError("Your location could not be found. Turn on the phone's Location/GPS and try again.");
        } else if (error.code === 3) {
          showLocationError("GPS took too long to respond. Make sure Location is on and try again.");
        } else {
          showLocationError("Unable to get your live location. Please try again.");
        }
      },
      {
  ...
  // This is the same basic action as the current-location/recenter button in Google Maps:
  // tap it -> ask for permission if necessary -> obtain fresh GPS -> place it in the field.
  locationBtn.addEventListener("click", getLiveLocation);
  locationInput.addEventListener("click", getLiveLocation);
  locationInput.addEventListener("focus", getLiveLocation);
  ...
  sosBtn.addEventListener("click", async () => {
    ...
    if (!gpsConfirmed || !Number.isFinite(gpsLatitude) || !Number.isFinite(gpsLongitude)) {
      err.textContent = "Your live GPS location is required. Tap the location button and allow location access before reporting the emergency.";
      err.style.display = "block";
      getLiveLocation();
      return;
    }
    ...
    if (!locationInput.value.trim()) {
      err.textContent = "Please confirm your location before reporting the emergency.";
      err.style.display = "block";
      return;
    }
    ...
    alert("Emergency report sent to Command Centre with your confirmed GPS location.");
    await render();
  });
}
