...
// Pending citizen emergency reports visible to Command Centre.
function getPendingCitizenReports() {
  return load().incidents.filter(i => i.status === "pending" && i.createdBy === "citizen");
}

// Remove and return one pending citizen report after Command Centre decides.
function removePendingCitizenReport(reportId) {
  const s = load();
  const index = s.incidents.findIndex(
    i => i.id === reportId && i.status === "pending" && i.createdBy === "citizen"
  );
  if (index === -1) return null;
  const [report] = s.incidents.splice(index, 1);
  save();
  return report;
}
...
module.exports = { load, save, getUser, getActiveIncident, getPendingCitizenReports, removePendingCitizenReport, addIncident, resolveActiveIncident, updateIncidentDepartmentStatus, addLog, getLog };
