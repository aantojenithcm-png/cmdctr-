...
async function renderCitizen() {
    ...
    <div class="citizen-wrap">
      ...
      <div style="display:grid; gap:8px; margin-bottom:12px;">
        ...
        <div style="display:grid; gap:7px;">
          <button type="button" id="citizen-location-btn" class="trigger-btn"
            style="width:100%; text-align:left; background:var(--panel2); border:1px solid var(--teal); color:var(--teal); font-weight:700;">
          ...
          <div id="citizen-location-status"
            ...
            Tap the location button to share your current GPS location. Google Maps will not open.
          </div>
        </div>
  ...
  // Get GPS only when the citizen explicitly presses the location button.
  ...
  const getLiveLocation = () => new Promise(resolve => {
    ...
    locationBtn.textContent = "📍 Finding your GPS location…";
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        locationBtn.textContent = "✓ GPS location captured — tap to refresh";
        locationStatus.textContent = `GPS location captured: ${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
        locationStatus.style.color = "var(--teal)";
        resolve({ latitude, longitude });
      },
      error => {
        locationBtn.disabled = false;
        locationBtn.textContent = "📍 Get my current GPS location";

        if (error.code === 1) {
          locationStatus.textContent = "Location permission was denied. Allow location access for this website and try again.";
        } else if (error.code === 2) {
          locationStatus.textContent = "Your GPS location could not be determined. Turn on Location/GPS and try again.";
        } else {
          locationStatus.textContent = "GPS location timed out. Please try again.";
        }
        locationStatus.style.color = "var(--amber)";
        resolve(null);
      },
