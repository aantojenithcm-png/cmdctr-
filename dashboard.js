...
async function renderCommand() {
    ...
    <div class="dpanel accent" style="margin-bottom:10px;">
      <div class="incident-top">
        <div>
          ...
          ${p.selectedHospitalName ? `
            <div style="font-size:13px; color:var(--teal); margin-top:7px; font-weight:700;">
              🏥 Selected hospital: ${p.selectedHospitalName}
            </div>
          ` : ""}
        </div>
      </div>
      ...
      <div style="display:flex; gap:8px; margin-top:12px;">
        <button class="trigger-btn" data-implement="${p.id}"
          ${p.selectedHospitalId ? "" : "disabled"}
          title="${p.selectedHospitalId ? "Hospital selected — click to implement emergency" : "Select a hospital first"}"
          style="background:var(--teal); color:#06231F; border:none; font-weight:700; opacity:${p.selectedHospitalId ? "1" : ".55"}; cursor:${p.selectedHospitalId ? "pointer" : "not-allowed"};">
          Implement emergency
        </button>
      ...
      target.querySelectorAll("[data-select-hospital]").forEach(selectBtn => {
        selectBtn.addEventListener("click", async () => {
          ...
          if (sr.status === 200) {
            ...
            const implementBtn = document.querySelector(`[data-implement="${selectBtn.dataset.reportId}"]`);
            if (implementBtn) {
              implementBtn.disabled = false;
              implementBtn.style.opacity = "1";
              implementBtn.style.cursor = "pointer";
              implementBtn.title = "Hospital selected — click to implement emergency";
            }
          } else {
  ...
  document.querySelectorAll("[data-implement]").forEach(btn => {
    btn.addEventListener("click", async () => {
      const hospitalId = selectedHospitalByReport[btn.dataset.implement];
      if (!hospitalId) {
        alert("Please select a hospital before implementing the emergency.");
        return;
      }
      ...
      const r = await apiPost(
        ...
        { reportId: btn.dataset.implement, decision: "approve", hospitalId },
        { auth: true, role: "command" }
      );
...
async function renderCitizen() {
      ...
      <div style="display:grid; gap:8px; margin-bottom:12px;">
        ...
        <div style="display:grid; gap:7px;">
          <input id="citizen-location"
            type="text"
            placeholder="📍 Tap here to detect your live location"
            readonly
            aria-label="Tap to detect your live GPS location"
            style="padding:12px 10px; border-radius:6px; border:1px solid var(--teal);
            background:var(--panel2); color:var(--text); cursor:pointer;">

          <div id="citizen-location-status"
            style="font-size:12px; color:var(--muted);">
            Tap the location field to automatically find your live GPS location. Google Maps will not open.
          </div>
        </div>
  ...
  // The LOCATION FIELD itself starts GPS detection when the citizen taps it.
  // Google Maps is NEVER opened by this action.
  ...
  const getLiveLocation = () => {
    ...
    locationRequestPromise = new Promise(resolve => {
      if (!navigator.geolocation) {
        locationStatus.textContent = "This browser does not support GPS location.";
        locationStatus.style.color = "var(--amber)";
        resolve(null);
        return;
      }

      locationInput.value = "📍 Finding your live location…";
      locationInput.style.borderColor = "var(--amber)";
      locationStatus.textContent = "Please allow location access when your browser asks.";
      locationStatus.style.color = "var(--amber)";
      ...
      navigator.geolocation.getCurrentPosition(
        position => {
          ...
          locationInput.dataset.locationSource = "gps";
          locationInput.style.borderColor = "var(--teal)";

          locationStatus.textContent = "✓ Live GPS location captured. Tap again to refresh.";
        ...
        error => {
          ...
          if (error.code === 1) {
            locationStatus.textContent = "Location permission was denied. Allow location access for this website and tap here again.";
          } else if (error.code === 2) {
            locationStatus.textContent = "GPS location could not be determined. Turn on Location/GPS and tap here again.";
          } else {
            locationStatus.textContent = "GPS location timed out. Tap here again to retry.";
          }
          locationStatus.style.color = "var(--amber)";
          resolve(null);
        },
  ...
  // Tapping/focusing the location field automatically requests live GPS.
  ...
  sosBtn?.addEventListener("click", async () => {
    ...
    // The emergency cannot be submitted without GPS coordinates.
    // If the citizen has not captured GPS yet, request it now without opening Maps.
    let latitude = locationInput?.dataset.latitude ? Number(locationInput.dataset.latitude) : null;
    let longitude = locationInput?.dataset.longitude ? Number(locationInput.dataset.longitude) : null;

    if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
      const gps = await getLiveLocation();
      if (!gps) {
        err.textContent = "Your GPS location is required before reporting the emergency.";
        err.style.display = "block";
